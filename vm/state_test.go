...
// TestBytesStateEntry - test byte array serialization of state entry
func TestBytesStateEntry(t *testing.T) {
	env := &Environment{EnableJIT: false, DefaultMemoryPages: 128, DefaultTableSize: 65536} // Init env
	...
	byteVal := stateEntry.Bytes() // Get byte value
...
// TestStringStateEntry - test string serialization of state entry
func TestStringStateEntry(t *testing.T) {
	env := &Environment{EnableJIT: false, DefaultMemoryPages: 128, DefaultTableSize: 65536} // Init env
	...
	stringVal := stateEntry.String() // Get string value

	if stringVal == "" { // Check for invalid string val
		t.Fatal("invalid serialized string value") // Panic
	}
