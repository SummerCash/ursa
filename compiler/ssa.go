...
func (c *SSAFunctionCompiler) FilterFloatingPoint() {
	for i, ins := range c.Code { // Iterate through provided instructions
		if strings.HasPrefix(ins.Op, "f32.") || strings.HasPrefix(ins.Op, "f64.") || // Check is float operation
			strings.HasSuffix(ins.Op, "/f32") || strings.HasSuffix(ins.Op, "/f64") { // Check / float operation
			if strings.Contains(ins.Op, ".reinterpret/") || strings.HasSuffix(ins.Op, ".const") { // Check is in whitelisted instruction space
				continue // whitelist
			}

			c.Code[i] = buildInstr(0, "fp_disabled_error", nil, nil) // Set has disabled float err
		}
	}
}
...
// buildInstr - initialize instruction with given params
func buildInstr(target TyValueID, op string, immediates []int64, values []TyValueID) Instr {
	return Instr{ // Return initialized instruction
		Target:     target,
		Op:         op,
